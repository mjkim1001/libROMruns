...
source _get_vecs.sh
myPath=../../../libROMruns/runnl0926Serial_rep
...
while [ 1 ]; do
    ...
    # Offline Phase 1
    nonlinear_elasticity_global_rom -offline -dt ${dt} -tf $t -s 14 -vs 100 | tee "${myPath}/nlel_offline1t${t}dt${dt}.txt" |
    ...
    # Merge Phase
    merge_output=$(nonlinear_elasticity_global_rom -merge -ns 1 -dt ${dt} -tf $t )
...
#for ((n = 0; n < 2 ; n++)); do
    ...
    srun -n 1 -p pdebug nonlinear_elasticity_global_rom -online -rxdim ${n_x[$n]} -rvdim ${n_v[$n]} -dt ${dt} -tf $t -eqp -ns 1 -hdim 1  > "${myPath}/nlel_eqp_online${n}t${t}dt${dt}.txt"
